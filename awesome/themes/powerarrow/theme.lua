...
theme.font                                      = "JetBrainsMono NerdFont 9"
theme.fg_normal                                 = fg
theme.fg_focus                                  = fg
theme.fg_urgent                                 = fg
theme.bg_normal                                 = bg
...
theme.fg_systray                                = bg_dim
theme.taglist_fg_focus                          = bg_dim
theme.taglist_bg_focus                          = fg_active
theme.taglist_bg_urgent                         = red
theme.taglist_fg_empty                          = green
theme.taglist_fg_occupied                       = green
theme.tasklist_bg_focus                         = bg
theme.tasklist_fg_focus                         = fg_active
...
local mem = lain.widget.mem({
    settings = function()
        widget:set_markup(markup.font(theme.font, "󰍛 " .. mem_now.used .. "MB "))
    end
})
...
local temp = lain.widget.temp({
    settings = function()
        widget:set_markup(markup.font(theme.font, "󰔏 " .. coretemp_now .. "°C "))
    end
})
--]]
    ...
    settings = function()
        if bat_now.status and bat_now.status ~= "N/A" then
            if bat_now.ac_status == 1 then
                baticon = "󰂄"
                widget:set_markup(markup.font(theme.font, baticon ..  " " .. bat_now.perc .. "% "))
                return
...
local net = lain.widget.net({
    settings = function()
        widget:set_markup(markup.fontfg(theme.font, " " .. net_now.received .. " ↓↑ " .. net_now.sent .. " "))
    end
})
...
function theme.at_screen_connect(s)
    ...
    s.mytaglist = awful.widget.taglist({
        ...
        style = {
            bg_urgent = bg_active,
            bg_volatile = bg_active,
            bg_empty = bg_active,
            bg_occupied = bg_active,
            bg_focus = fg_active,
            spacing = 4,
        },
    ...
    -- Create the wibox
    s.mywibox = awful.wibar({ position = "top", screen = s, height = dpi(16), bg = theme.bg_normal, fg = bg_dim })
    ...
    s.mywibox:setup {
        ...
        { -- Right widgets
            ...
            pl(wibox.widget { mem.widget, layout = wibox.layout.align.horizontal }, cyan),
            pl(wibox.widget { cpu.widget, layout = wibox.layout.align.horizontal }, magenta),
            --pl(wibox.widget { temp.widget, layout = wibox.layout.align.horizontal }, magenta),
            --pl(wibox.widget { fsicon, theme.fs and theme.fs.widget, layout = wibox.layout.align.horizontal }, "#CB755B"),
            pl(wibox.widget { bat.widget, layout = wibox.layout.align.horizontal }, blue),
            --pl(wibox.widget { neticon, net.widget, layout = wibox.layout.align.horizontal }, color6),
            pl(wibox.widget.textclock(" %d.%m %H:%M ",60), green),
            --]]
            --[[ using separators
